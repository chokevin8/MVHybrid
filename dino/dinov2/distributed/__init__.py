...
def get_local_rank() -> int:
    ...
    """
    if is_enabled():
        print("Multi GPU node training detected, using local rank.")
    if not is_enabled():
        return 0
...
class _TorchDistributedEnvironment:
    def __init__(self):
        ...
        self.visible_cuda_devices = "0, 1, 2, 3, 4, 5, 6, 7"

        if _is_slurm_job_process():
            return self._set_from_slurm_env()
        env_vars = _collect_env_vars()
        print(f'len of env_vars is:"{len(env_vars)}"')
        print(f'len of torch_distributed_env_vars is:"{len(_TORCH_DISTRIBUTED_ENV_VARS)}"')
        ...
        if not env_vars:
            ...
            print("Environment is fully set!")
    ...
    def _set_from_preset_env(self):
        ...
        print("Initializing from preset environment, ex. torchrun.")
...
def enable(*, set_cuda_current_device: bool = True, overwrite: bool = False, allow_nccl_timeout: bool = False):
    ...
    print(f'Local rank is: "{_LOCAL_RANK}"')
    print(f'Local world size is: "{_LOCAL_WORLD_SIZE}"')

    if _LOCAL_RANK >= 0 or _LOCAL_WORLD_SIZE >= 0:
        raise RuntimeError("Distributed mode has already been enabled")
    print("DDP environment detected.")
