...
import sys
sys.path.append('/home/wjcho/miccai/MambaVision/mambavision/models')  
# sys.path.append('/scratch/x2946a13/miccai/MambaVision/mambavision/models')  
from modules import get_INR
...
class MambaVisionMixer(nn.Module):
    def __init__(
        ...
        # repeated numebr from 1 to self.d_state repeated for self.d_inner/2 times
...
class Block(nn.Module):
    def __init__(self, 
        ...
        if is_transformer:
            ...
            self.channel_mixer = get_INR(
                mode='relu+fr',  # or 'sin+fr' if you prefer
                in_features=dim,
                hidden_features=dim,
                hidden_layers=1,
                out_features=dim,
...
class MambaVision(nn.Module):
    ...
    def forward_features(self, x, masks = None):
        ...
        #add position embeddings (commented b/c no pos embed needed)
        # x = x + self.pos_embed
        ...
        return {
        # after mamba impl, first flatten [B,num_cls_token,D] to [B,num_cls_token*D]
        "x_norm_clstoken": x[:, token_position].view(B, -1),
        "x_norm_patchtokens": torch.cat((x[:, :token_position], x[:, token_position + 1:]), dim=1),
        "masks": masks,
        } 
...
@register_pip_model
@register_model
def frmlp_mambavision_S(**kwargs):
    # model_path = kwargs.pop("model_path", "/tmp/mamba_vision_S.pth.tar")
    embed_dim = kwargs.pop("embed_dim", 384) # 384/768
    ...
    pretrained_cfg = resolve_pretrained_cfg('mamba_vision_S_FR').to_dict()
...
# PYTHONPATH=/scratch/x2946a13/miccai python -c "import torch; from MambaVision.mambavision.models.mamba_vision_flat_fr import mamba_vision_S_FR; model = mamba_vision_S_FR(img_size=256, depth=24); print(model); print(f'Parameters: {sum(p.numel() for p in model.parameters())/1e6:.2f}M')"
# 27.12 M
# if depth = 12, 13.71M
