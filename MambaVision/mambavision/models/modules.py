...
class Fourier_reparam_linear(nn.Module):
     # 384,384,32,32,6 self.net.append(Fourier_reparam_linear(hidden_features,hidden_features,high_freq_num,low_freq_num,phi_num,alpha))
    ...
    def forward(self,x):
        weight=torch.matmul(self.lamb,self.bases) # 3,384 x 384,2 
        output=torch.matmul(x,weight.transpose(0,1)) # 2 x 3
...
class INR(nn.Module):
    ...
    phi_num,alpha,first_omega_0,hidden_omega_0,pe):
        ...
        if mode=='relu+fr':
            self.net.append(nn.Linear(in_features,hidden_features)) # 2, 384
            self.net.append(nn.ReLU())
            for i in range(hidden_layers): # 384,384,32,32,6
